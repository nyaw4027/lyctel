...
def add_missing_columns(apps, schema_editor):
    """
    Safely add every Order column that may be missing from Railway's PostgreSQL.
    Uses IF NOT EXISTS pattern via information_schema — never crashes if column exists.
    """
    columns = [
        # (name,  DDL type,                         default value for NOT NULL cols)
    ...
    with connection.cursor() as cursor:
        ...
        for col, col_type, default in columns:
            if col not in existing:
                if default is None:
                    cursor.execute(
                        f'ALTER TABLE "order_order" ADD COLUMN "{col}" {col_type} NULL'
                    )
                else:
                    cursor.execute(
                        f'ALTER TABLE "order_order" ADD COLUMN "{col}" {col_type} NOT NULL DEFAULT {default}'
                    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        # Step 1: actually create the DB columns where missing
        migrations.RunPython(add_missing_columns, noop),

        # Step 2: sync Django's migration state (no DB ops — already done above)
            ...
            state_operations=[
                migrations.AddField(
                    ...
                    field=models.CharField(
                        choices=[('rider','Rider Delivery'),('pickup','Self Pickup'),('parcel','Bus / Parcel')],
                        default='rider', max_length=10,
                    ),
                ),
                migrations.AddField(
                    model_name='order',
